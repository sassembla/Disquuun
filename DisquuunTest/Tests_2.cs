...
public partial class Tests {
	...
	public void _2_0_1_AddJob_Async_TimeToLive (Disquuun disquuun) {
		WaitUntil("_2_0_1_AddJob_Async_TimeToLive", () => (disquuun.State() == Disquuun.ConnectionState.OPENED), 5);
		
		var queueId = Guid.NewGuid().ToString();
		
		var jobId = string.Empty;
		disquuun.AddJob(queueId, new byte[10], 0, "TTL", 100).Async(
			(command, result) => {
				jobId = DisquuunDeserializer.AddJob(result);
			}
		);
	...
	public void _2_0_2_AddJob_Async_TimeToLive_Wait_Dead (Disquuun disquuun) {
		WaitUntil("_2_0_2_AddJob_Async_TimeToLive_Wait_Dead", () => (disquuun.State() == Disquuun.ConnectionState.OPENED), 5);
		
		var queueId = Guid.NewGuid().ToString();
		
		var jobId = string.Empty;
		disquuun.AddJob(queueId, new byte[10], 0, "TTL", 1).Async(
			(command, result) => {
				jobId = DisquuunDeserializer.AddJob(result);
			}
		);
	...
	public void _2_0_3_AddJob_Async_Retry (Disquuun disquuun) {
		WaitUntil("_2_0_3_AddJob_Async_Retry", () => (disquuun.State() == Disquuun.ConnectionState.OPENED), 5);
		
		var queueId = Guid.NewGuid().ToString();
		
		var jobId = string.Empty;
		disquuun.AddJob(queueId, new byte[10], 0, "RETRY", 1).Async(
			(command, result) => {
				jobId = DisquuunDeserializer.AddJob(result);
			}
		);
	...
	public void _2_0_4_AddJob_Async_Retry_0_And_TTL_1Sec (Disquuun disquuun) {
		WaitUntil("_2_0_4_AddJob_Async_Retry_0_And_TTL_1Sec", () => (disquuun.State() == Disquuun.ConnectionState.OPENED), 5);
		
		var queueId = Guid.NewGuid().ToString();
		
		var jobId = string.Empty;
		disquuun.AddJob(queueId, new byte[10], 0, "RETRY", 0, "TTL", 1).Async(
			(command, result) => {
				jobId = DisquuunDeserializer.AddJob(result);
			}
		);
